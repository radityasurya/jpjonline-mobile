...
import { activityStorage, examStorage, bookmarkStorage, statsCalculator } from '@/utils/storage';
...
export const notesAPI = {
  ...
  async fetchNotes(category?: string, search?: string): Promise<ApiResponse<Note[]>> {
    await delay(1000);
...
export const examsAPI = {
  ...
  async fetchExams(userSubscription: 'free' | 'premium' = 'free'): Promise<ApiResponse<Exam[]>> {
    await delay(1000);
  ...
  async fetchExamQuestions(examId: string): Promise<ApiResponse<{ exam: Exam; questions: Question[] }>> {
    await delay(800);
...
export const authAPI = {
  ...
  async register(userData: RegisterRequest): Promise<ApiResponse<User>> {
    ...
    const existingUser = usersData.demoUsers.find(u => u.email === userData.email);
...
export const contactAPI = {
  async submitContactForm(formData: ContactRequest): Promise<ApiResponse<string>> {
    await delay(2000);
...
export const storageAPI = {
  ...
  async saveActivity(activity: Omit<Activity, 'id' | 'timestamp'>): Promise<ApiResponse<Activity>> {
    await delay(100);
